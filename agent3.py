...
class Agent_3(Agent):
    def __init__(self, aid):
        super(Agent_3, self).__init__(aid=aid, debug=False)
        self.pos = [0,0]
        self.back = 0
        self.escaped = 0
    ...
    def on_start(self):
        super(Agent_3, self).on_start()
        display_message(self.aid.localname, "Recherche de la sortie...")
        self.call_later(20.0, self.escape)
    def sending_pos(self):
        #display_message(self.aid.localname, "Dead end found : sending to agent_1")
        message = ACLMessage(ACLMessage.PROPOSE)
        message.set_protocol(ACLMessage.FIPA_REQUEST_PROTOCOL)
        message.set_sender(AID('agent_3'))
        message.add_receiver(AID('agent_1'))
    ...
    def react(self, message):
        super(Agent_3, self).react(message)
        perPRO="propose"
        ontoPRO="deadend"
